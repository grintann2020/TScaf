...
namespace T {
    ...
    public struct STtTwn : ITwn {
        ...
        public bool IsEnd { get { return _stp == _nOI - 1; } } // get is end or not
        ...
        private int _nOI; // number of interval
        private int _stp; // step
        private int _tmpStp; // temp step
        private bool _isDps; // is deposable

        public STtTwn(int id, ITrc[] iTrcArr, int nOI, float drtn, float dly, float tm, bool isExct = true, bool isDps = true, DAct dEnd = null) {
            _id = id;
            _iTrcArr = iTrcArr;
            _dEnd = dEnd;
            _nOI = nOI;
        ...
        private int CmlStp(float tm) { // current cumulative 
            return (int)Math.Floor(CrrTm(tm) / _drtn * _nOI);
        }
